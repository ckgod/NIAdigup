# 1.2.1 Version Catalog

## Version Catalog
Gradle 7.0부터 도입된 기능으로, 프로젝트 의존성과 플러그인 버전을 중앙에서 관리할 수 있게 해주는 매커니즘이다.

<code>libs.versions.toml</code> 파일에 작성하며, TOML(Tom's Obvious, Minimal Language) 형식을 사용한다. <a href="https://ko.wikipedia.org/wiki/TOML">TOML 위키</a>


### 장점
- 중앙 집중화: 모든 의존성과 버전을 한 곳에서 관리할 수 있다.
- 일관성: 프로젝트 전체에서 동일한 버전의 라이브러리를 사용할 수 있다.
- 가독성: 빌드 스크립트가 간결해지고 이해하기 쉬워진다.
- 유지보수: 버전 업데이트가 용이해진다.
- IDE 지원: 자동완성 기능을 통해 의존성 추가가 쉬워진다.

### 문법

#### [versions]

프로젝트에서 사용되는 라이브러리와 플러그인의 버전을 정의한다.

예: <code>accompanist = "0.34.0"</code>


#### [libraries]

프로젝트에서 사용할 라이브러리를 정의한다.
각 라이브러리는 group, name, version을 가진다.
버전은 [versions] 섹션에서 정의한 값을 참조할 수 있다.

예: <code>androidx-activity-compose = { group = "androidx.activity", name = "activity-compose", version.ref = "androidxActivity" }</code>


#### [bundles]

여러 라이브러리를 하나의 번들로 그룹화한다.

예: <code>androidx-compose-ui-test = ["androidx-compose-ui-test", "androidx-compose-ui-testManifest"]</code>


#### [plugins]

프로젝트에서 사용할 Gradle 플러그인을 정의한다.
각 플러그인은 id와 version을 가진다.

예: <code>android-application = { id = "com.android.application", version.ref = "androidGradlePlugin" }</code>
